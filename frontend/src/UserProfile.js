...
const MOCK_BRACKETS = [
  { id: 1, name: "NFL Playoffs 2025", sport: "NFL",  status: "active",    createdAt: "2025-01-10" },
  { id: 2, name: "March Madness",     sport: "NCAA", status: "active",    createdAt: "2025-03-14" },
  { id: 3, name: "NBA Finals Run",    sport: "NBA",  status: "completed", createdAt: "2025-04-01" },
  { id: 4, name: "World Cup Group A", sport: "FIFA", status: "completed", createdAt: "2024-11-20" },
];
    ...
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "aliceblue", padding: "16px 20px", borderBottom: "1px solid #e5e7eb", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <Link to="/profile" className="btn" style={{ background: "#4a429f", color: "#fff", padding: "8px 16px", borderRadius: 5, fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
            My Profile
          </Link>
      ...
      <main style={{ maxWidth: 600, margin: "0 auto", padding: "28px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={S.card}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            {/* Avatar */}
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#4a429f", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
              {initials}
            </div>

            {/* Info — read only, no editing */}
            <div>
              <p style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#1f2937" }}>
                {profile.username}
              </p>
              ...
              {profile.favoriteTeam && (
                <p style={{ margin: "6px 0 0", fontSize: 13, fontWeight: 600, color: "#1f2937" }}>
                  ⭐ {profile.favoriteTeam}
                </p>
              )}
            </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
            <div style={S.pill("#f3f4f6", "#374151")}>{total} brackets total</div>
            <div style={S.pill("#f0fdf4", "#15803d")}>{completed} completed</div>
        ...
        <div style={S.card}>
          ...
          {brackets.length === 0 ? (
            <p style={{ color: "#9ca3af", fontSize: 13, textAlign: "center", padding: "20px 0" }}>
              No brackets yet. <Link to="/dashboard" style={{ color: "#4a429f", fontWeight: 600 }}>Create one!</Link>
            </p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {brackets.map((b, i) => {
                const c       = SPORT_COLORS[b.sport] || { bg: "#f9fafb", text: "#374151", border: "#e5e7eb" };
